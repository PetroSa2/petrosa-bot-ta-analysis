...
def test_signal_serialization(signals):
    ...
    for i, signal in enumerate(signals):
        ...
        required_fields = [
            'strategy_id', 'symbol', 'action', 'confidence', 'current_price',
            'price', 'source', 'strategy', 'timeframe', 'order_type', 'time_in_force'
        ]
...
def test_signal_validation(signals):
    ...
    for i, signal in enumerate(signals):
        ...
        logger.info(f"Signal {i+1} validation: {'✅ Valid' if is_valid else '❌ Invalid'}")

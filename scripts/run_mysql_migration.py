    ...
    try:
        ...
        try:
            with connection.cursor() as cursor:
                ...
                logger.info(
                    "Updating existing NULL/empty 'timeframe' values to default '15m'..."
                )
                cursor.execute(
                    "UPDATE signals SET timeframe = '15m' WHERE timeframe IS NULL OR timeframe = ''"
                )
                logger.info("✓ Existing 'timeframe' values updated")

                # Ensure an index exists on signals(timeframe)
                logger.info("Checking for existing index 'idx_signals_timeframe'...")
                cursor.execute(
                    """
                ...
                if row:
                    if isinstance(row, dict):
                        idx_exists = row.get("idx_exists", 0) > 0
                    else:
                        idx_exists = row[0] > 0

                if not idx_exists:
                    logger.info("Creating index 'idx_signals_timeframe'...")
                    cursor.execute(

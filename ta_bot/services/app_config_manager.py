...
class AppConfigManager:
    ...
    async def get_previous_config(self) -> AppConfigAudit | None:
        """Get immediately previous configuration version."""
    ...
    async def rollback_config(
        ...
        success, restored_config, set_errors = await self.set_config(
            parameters=config_to_restore,
            changed_by=changed_by,
            reason=f"Rollback: {reason} (from {target_audit.id})",
            validate_only=False,
        )
        if not success:
        ...
        logger.info(
            f"Configuration rolled back to {target_audit.id}", extra={"reason": reason}
        )

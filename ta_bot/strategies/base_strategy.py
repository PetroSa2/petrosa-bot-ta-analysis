...
class BaseStrategy(ABC):
    ...
    def _get_current_values(self, indicators: Dict[str, Any], df: pd.DataFrame) -> Dict[str, float]:
    ...
    def _get_previous_values(self, indicators: Dict[str, Any], df: pd.DataFrame) -> Dict[str, float]:
    ...
    def _check_cross_above(self, current: float, previous: float, threshold: float = 0) -> bool:
        """Check if a value crossed above a threshold."""
        return previous <= threshold and current > threshold
    
    def _check_cross_below(self, current: float, previous: float, threshold: float = 0) -> bool:
